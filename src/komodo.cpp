    ...
    if ( scriptbuf[len++] == 0x6a ) // if this is an opreturn vout
    {
        ...
        if ( chainName.isKMD() )
        {
            if ( strcmp("KMD",(char *)&scriptbuf[len+32 * 2 + 4]) == 0 )
                matched = 1;   // it is a back nota is for this chain (KMD)
        }
        else
        {
            if ( scriptbuf[len] == 'K' ) // this key-value feature is deprecated and should not be used
            {
            ...
            if ( strcmp(chainName.symbol().c_str(),(char *)&scriptbuf[len+32*2+4]) == 0 )
                matched = 1;  // it is a back nota is for this chain (asset chain)
        }
        offset = 32 * (1 + matched) + 4;
        nameoffset = (int32_t)strlen((char *)&scriptbuf[len+offset]);
        nameoffset++;
        memset(&ccdata,0,sizeof(ccdata));
        strncpy(ccdata.symbol,(char *)&scriptbuf[len+offset],sizeof(ccdata.symbol)); // read notarised chain name
        if ( j == 1 && opretlen >= len+offset-opoffset )
        {
            ...
            len += iguana_rwnum(0,&scriptbuf[len],sizeof(*notarizedheightp),(uint8_t *)notarizedheightp); // read noatrised height
            if ( matched != 0 )
            ...
            if ( notarized != 0 && validated != 0 )
            {
                ...
                if ( matched != 0 && *notarizedheightp > sp->LastNotarizedHeight() && *notarizedheightp < height )
                {
                    ...
                    if ( chainName.isKMD() )
                    {
                        ...
                        if ( opretlen > len && scriptbuf[len] == 'A' ) // 'A' not used and not saved
                        {
        ...
        else if ( matched != 0 )
        {
            ...
            // Note: 'X' and 'A' not used and not saved
